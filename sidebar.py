...
def ai_scoring_tab(title, prompt, key):
    ...
    if use_ai:
        ...
        elif tit        elif title == "Finance Seekers":
            s1 = st.checkbox("Project proposals are well developed and aligned with climate finance needs", key=f"{key}_seek_s1")     s2 = st.checkbox("A pipeline of climate projects is available for financing", key=f"{key}_seek_s2")
