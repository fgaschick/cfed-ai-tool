...
def ai_scoring_tab(title, prompt, key):
    ...
    if use_ai:
        ...
        if title == "Enabling Environment":
            s1 = st.checkbox("Country has submitted an NDC", value=st.session_state.get(f"{key}_env_s1", False), key=f"{key}_env_s1")
            s2 = st.checkbox("NDC is linked to investment or implementation plans", value=st.session_state.get(f"{key}_env_s2", False), key=f"{key}_env_s2")
            s3 = st.checkbox("NDC or strategy includes financing targets or mechanisms", value=st.session_state.get(f"{key}_env_s3", False), key=f"{key}_env_s3")
            s4 = st.checkbox("There is a national climate finance strategy or roadmap", value=st.session_state.get(f"{key}_env_s4", False), key=f"{key}_env_s4")
        elif title == "Ecosystem Infrastructure":
            s1 = st.checkbox("Physical infrastructure for climate adaptation and mitigation exists", value=st.session_state.get(f"{key}_infra_s1", False), key=f"{key}_infra_s1")
            s2 = st.checkbox("There is a national or regional data infrastructure for monitoring climate impacts", value=st.session_state.get(f"{key}_infra_s2", False), key=f"{key}_infra_s2")
            s3 = st.checkbox("Climate-related digital platforms are available for stakeholders", value=st.session_state.get(f"{key}_infra_s3", False), key=f"{key}_infra_s3")
            s4 = st.checkbox("Regulatory frameworks for climate finance and development are in place", value=st.session_state.get(f"{key}_infra_s4", False), key=f"{key}_infra_s4")
        elif title == "Finance Providers":
            s1 = st.checkbox("Public finance providers are operational and engaged in climate finance", value=st.session_state.get(f"{key}_prov_s1", False), key=f"{key}_prov_s1")
            s2 = st.checkbox("Private finance providers are actively engaged in climate finance", value=st.session_state.get(f"{key}_prov_s2", False), key=f"{key}_prov_s2")
            s3 = st.checkbox("Development finance institutions provide substantial climate finance", value=st.session_state.get(f"{key}_prov_s3", False), key=f"{key}_prov_s3")
            s4 = st.checkbox("Multilateral development banks are active in the climate finance ecosystem", value=st.session_state.get(f"{key}_prov_s4", False), key=f"{key}_prov_s4")
        elif title == "Finance Seekers":
            s1 = st.checkbox("Project proposals are well developed and aligned with climate finance needs", value=st.session_state.get(f"{key}_seek_s1", False), key=f"{key}_seek_s1")
            s2 = st.checkbox("A pipeline of climate projects is available for financing", value=st.session_state.get(f"{key}_seek_s2", False), key=f"{key}_seek_s2")
            s3 = st.checkbox("There is easy access to finance for climate-related projects", value=st.session_state.get(f"{key}_seek_s3", False), key=f"{key}_seek_s3")
            s4 = st.checkbox("Stakeholder engagement is integral to project development", value=st.session_state.get(f"{key}_seek_s4", False), key=f"{key}_seek_s4")
